...
import { generateReceiptHtml, generateRequestLetterHtml, generateTranscriptHtml } from "./generateReceiptHtml.js";
...
export async function generateRequestLetterPdfBuffer(data) {

  const html = await generateRequestLetterHtml(data);
  const file = { content: html }; 
  const options = { format: 'A4' };
